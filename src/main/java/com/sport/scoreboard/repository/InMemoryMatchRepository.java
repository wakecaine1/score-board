...
public class InMemoryMatchRepository implements MatchRepository {
    ...
    public Match save(Match match) {
        ongoingMatches.put(key(match.getHomeTeam().getName(), match.getAwayTeam().getName()), match);
        return match;
    }

    @Override
    public void delete(Match match) {
        ongoingMatches.remove(key(match.getHomeTeam().getName(), match.getAwayTeam().getName()));
    }
